...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("debug_v13.log", encoding='utf-8'),
            logging.StreamHandler(sys.stdout)
        ]
    )
    logging.info("V13.py started.")
        ...
        while IS_RUNNING:
            try:
                ...
                if sleep_time > 0:
                    ...
                    logging.warning(f"Loop running slow. Elapsed: {elapsed_time:.4f}s")

            except Exception as e:
                logging.error(f"Error in main loop: {e}")
                logging.error(traceback.format_exc())
                # エラー発生時も続行するか、終了するか。ここでは安全のため終了させるが、ログには残る。
                IS_RUNNING = False
