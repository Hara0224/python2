...
peak_val = -np.inf
peak_time = None
last_direction = None  # 前回HITの方向を記憶
...
def on_emg(emg, movement):
    ...
    if trigger_time is None:
        ...
        if (t_now - trigger_time) * 1000.0 > PEAK_WIN_MS:
            ...
            last_direction = direction  # 前回方向更新
            trigger_time = None
            trigger_ch = None
            direction = None
            peak_val = -np.inf
...
def send_motor_command(direction, t_trigger):
    ...
    t_hit_pc = time.time()  # PCでHIT判定した時間
    ser.write((cmd + "\n").encode())
    t_send = time.time()  # Arduino送信時間

    hit_time = None
    while time.time() - t_send < MAX_MOTOR_WAIT:
        line = ser.readline().decode().strip()
        if line.startswith("HIT"):
            hit_time = time.time()  # Arduino応答時間
            break

    if hit_time:
        sw_delay = (t_hit_pc - t_trigger) * 1000
