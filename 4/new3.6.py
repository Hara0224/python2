...
def measure_sensor_delay():
    ...
    while time.time() - start < MAX_SENSOR_WAIT:
        line = ser_sensor.readline().decode().strip()
        if not line:
            continue
        try:
            if line.startswith("ACC1:"):
                acc1_delta = int(line.split(":")[1])
            elif line.startswith("ACC2:"):
                acc2_delta = int(line.split(":")[1])
        except:
            continue
...
try:
    ...
    for f in [csv_file, raw_csv_file, rms_csv_file, ema_csv_file]:
        try:
            f.close()
        except:
            pass
    for s in [ser_motor, ser_sensor]:
        try:
            s.close()
        except:
            pass
