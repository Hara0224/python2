...
CHANNELS = [2, 3, 6, 7]
RAW_FS = 200
WINDOW_MS = 50  # V10で使用したWindow Size
WINDOW_SAMPLES = int(RAW_FS * WINDOW_MS / 1000)  # 10 サンプル

# V7/V9学習時に計算された値を使用
# NOTE: T_ONSETの値はV7/V9の学習ログ (35.6482) を使用
T_ONSET = 35.6482
...
def plot_emg_onset_analysis(sample_label="radial_dev", num_trials=3):
    ...
    for i, file_path in enumerate(file_list[:num_trials]):
        ...
        # RMSは50msウィンドウ/50msステップで計算 (時系列の形状を明確にするため)
        step = WINDOW_SAMPLES  # 50msステップ
        ...
        for j in range(0, len(selected_emg) - WINDOW_SAMPLES + 1, step):
            ...
            time_points.append((j + WINDOW_SAMPLES / 2) / RAW_FS)  # ウィンドウ中央時刻

        ax = axes[i, 0]
        # 点のみのプロット (50msごとの離散特徴量であることを示す)
        ax.plot(
            time_points,
            rms_timeseries,
            label="Max RMS (4ch)",
