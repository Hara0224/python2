...
plt.rcParams["font.size"] = 30  # 文字サイズを大きめに
...
def main():
    ...
    try:
        ...
        # EMGデータ (ch1-ch8)
        # 選択されたチャンネルのみループ
        # EMG用固定カラーパレット (Tab10をベースに調整)
        # ch1:Blue, ch2:Orange, ch3:Green, ch4:Red, ch5:Purple, ch6:Brown, ch7:Pink, ch8:Gray
        ...
        for i in selected_ch_indices:
            ...
            if col in df.columns:
                ...
                # プロット
                lines = ax.plot(df["time_rel"], df[col], label=f"筋電 {col}", alpha=1.0, linewidth=1.5, color=c)
        ...
        if is_down_ch_selected:
            # Strong Thresholdは目立つ色で (Black dashed)
            ax.axhline(y=strong_threshold, color="black", linestyle="--", linewidth=1.5, label=f"強閾値 ({strong_threshold:.1f})")

        # 振動データ (vib1_z, vib2_z)
        # 軸を分ける
        ax2 = ax.twinx()

        # VibrationはEMGと被らない色にする (Cyan, Oliveなど) -> 青・オレンジに変更
        ...
        for col in selected_vib_cols:
            if col in df.columns:
                ...
                ax2.plot(df["time_rel"], v_data, label=f"振動 {col}", linestyle="-", linewidth=1.5, alpha=0.8, color=c)
        ...
        # 最初の17秒（キャリブレーション期間）を表示しない
        ax.set_xlim(left=calib_strong_end)
        plt.show()
